...
static void init_ardupilot()
{
	...
	mavlink_system.sysid = (uint8_t)MAV_SYSTEM_ID;				// Using g.sysid_this_mav
	mavlink_system.compid = (uint8_t)MAV_GCS_ID;					// We do not check for comp id
	...
	// Force AP to start in MANUAL mode and NOT in mode selected in first bin
	// with new Safety MUX being used, first mode is now AUTO
	set_mode(MANUAL);		//set_mode(flight_modes[0]);
	...
	gcs_send_text_P(SEVERITY_LOW,PSTR(THISVEHICLE));
	gcs_send_text_P(SEVERITY_LOW,PSTR(THISFLIGHTMODE));
	gcs_send_text_P(SEVERITY_LOW,PSTR(SLV_GIT_CODE));
	//gcs_send_text_P(SEVERITY_LOW,PSTR("Four D Waypoints"));
	//gcs_send_text_P(SEVERITY_LOW,PSTR("Turn Anticipation Enabled \n\n"));
	//gcs_send_text_P(SEVERITY_LOW,PSTR(THISFIRMWARE));
	//Serial.printf_P(PSTR("\n\n"));
	//Serial3.printf_P(PSTR("\n\n"));
}
...
{
	// If mode equals current control mode, do nothing
	if(control_mode == mode){
		// don't switch modes if we are already in the correct mode.
		return;
	}

	// Check current control mode and perform necessary 
	// functions to switch from current mode into new mode safely
	if(g.auto_trim > 0 && control_mode == MANUAL)
		trim_control_surfaces();

	if (control_mode == AUTO) //if switching out of AUTO mode, clear 4D mode and vector command
	{
	...
	// Enable this code if going HOME issue reappears after
	// switching modes (AUTO/MANUAL/AUTO)
	/*
	if ((control_mode == AUTO) && (mode == MANUAL))
	{
		change_command((uint8_t) 1);
	}
	if ((control_mode == MANUAL) && (mode == AUTO))
	{
		change_command((uint8_t) 2);
	}
	*/

	// Switch current control mode to mode and perform 
	// necessary functions to switch to new mode safely
	control_mode = mode;
	crash_timer = 0;
	...
	switch(control_mode)
	{
		...
		case FLY_BY_WIRE_A:
		case FLY_BY_WIRE_B:
			//target_altitude = current_loc.alt - home.alt;
			target_altitude = current_loc.alt;
			//target_altitude = 20000;	// 200 meters
			break;
