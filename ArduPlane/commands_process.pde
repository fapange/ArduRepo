...
static void change_command(uint8_t cmd_index)
{
	...
	if (temp.id > MAV_CMD_NAV_LAST ){
		...
		//nav_command_index 	= cmd_index; // trying to eliminate problem of wpt number decrementing by one when switching between auto and manual modes
...
static void process_next_command()
{
	...
	struct Location temp2;
	byte wp_count;
	byte old_index = 0;

	// these are Navigation/Must commands
	// ---------------------------------
	if (nav_command_ID == NO_COMMAND)
	{ // no current navigation command loaded
		old_index = nav_command_index;
		temp.id = MAV_CMD_NAV_LAST; // Marks the ENUM end of navigational commands only conditional and do commands follow
		while(temp.id >= MAV_CMD_NAV_LAST && nav_command_index <= g.command_total) 
		{ // while there are commands left, and they are conditional or do commands
			nav_command_index++;

			temp = get_cmd_with_index(nav_command_index);
			if (temp.id == MAV_CMD_DO_JUMP)		// added to find next waypoint in case there is a jump command
			{
		...
		temp2.id = MAV_CMD_NAV_LAST;
		nav_command_index2 = nav_command_index;
		wp_count = 0;
		while(temp2.id >= MAV_CMD_NAV_LAST && wp_count <= g.command_total) 
		{
			wp_count++;
			nav_command_index2++;
			//if (nav_command_index2 > g.command_total)
			//	nav_command_index2 = 0;
			temp2 = get_cmd_with_index(nav_command_index2);
			if (temp2.id == MAV_CMD_DO_JUMP)	// added to find next waypoint in case there is a jump command
			{
	...
	// -------------------------------------------
	if (non_nav_command_index == NO_COMMAND) 
	{		// If the index is NO_COMMAND then we have just loaded a nav command
	...
	{	// If the ID is NO_COMMAND then we have just completed a non-nav command
