...
static long read_alt_to_hold()
{
	if(g.RTL_altitude < 0)
		...
		return g.RTL_altitude;	// return g.RTL_altitude + home.alt;
}
...
{
	modify_next_WP(wp, wp->p1);	// Load the next_WP slot
}
/*
This function stores waypoint commands
It looks to see what the next command type is and finds the last command.
...
static void modify_next_WP(struct Location *wp, int delta)
{
	...
	// ---------------------
	next_WP = *wp;
	next_WP2 = next_nav_command2;

	if (b_4DWaypointsEnabled() && b_4Dflag && (nav_command_index>0))
	{
		// 4D mode is active
		Serial.printf_P(PSTR("[%1d:%1d:%d] ")
							,(int)b_4DWaypointsEnabled()
							,(int)b_4Dflag
							,(int)nav_command_index);
		Serial.print(time_left);
		Serial.print(" + ");
		Serial.print((float)(delta));
		Serial.print(" = ");
	
		if (next_WP.p1 > 0)
		{
			time_left += (float)(delta);
			if (time_left > 0)
				gcs_send_text_fmt(PSTR("Going to Waypoint %i in %d sec"),nav_command_index, (int)time_left);
			else
				gcs_send_text_fmt(PSTR("Late to Waypoint %i by %d sec"),nav_command_index, abs((int)time_left));
		}
...
static void set_guided_WP(void)
{
	...
	// used to control FBW and limit the rate of climb
	// -----------------------------------------------
	target_altitude = next_WP.alt; // - home.alt;
	offset_altitude = next_WP.alt - prev_WP.alt;
