...
#if SLV_RC_CHANNEL_FILTER == ENABLED
	//pwm = constrain(pwm,(int)radio_min.get(),(int)radio_max.get());
	fpwm_in = ((double)pwm + 7.0*fpwm_in)/8.0;		// Small filtering
	radio_in = (int)fpwm_in;
#else
	#error SLV_RC_CHANNEL_FILTER DISABLED.
	if(_filter){
		if(radio_in == 0)
			radio_in = pwm;
		else
...
RC_Channel::norm_output()
{
	if(radio_out < radio_trim)
		return _reverse * (float)(radio_out - radio_trim) / (float)(radio_trim - radio_min);
	else
		return _reverse * (float)(radio_out - radio_trim) / (float)(radio_max  - radio_trim);
}
