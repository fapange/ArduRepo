 ...
 */
#if MAVLINK_CRC_EXTRA
MAVLINK_HELPER void _mav_wID_finalize_message_chan_send(mavlink_channel_t chan, uint8_t sID, uint8_t cID, uint8_t msgid, const char *packet, 
						    uint8_t length, uint8_t crc_extra)
#else
MAVLINK_HELPER void _mav_wID_finalize_message_chan_send(mavlink_channel_t chan, uint8_t sID, uint8_t cID, uint8_t msgid, const char *packet, uint8_t length)
#endif
{
	uint16_t checksum;
	uint8_t buf[MAVLINK_NUM_HEADER_BYTES];
