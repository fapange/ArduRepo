                            ...
                            void *               inRefcon)
{
	if (Globals::XplanesSEND->Available > 0) {
		...
		XPLMWorldToLocal(lat,lng,alt * kFtToMeters,&X,&Y,&Z);
			...
			XPLMSetDataf(gPlaneX, (float)X);
			XPLMSetDataf(gPlaneY, (float)Y);
			XPLMSetDataf(gPlaneZ, (float)Z);

			
			XPLMSetDataf(gPlaneTheta, (float)pitch);
			XPLMSetDataf(gPlanePhi, (float)roll);
			XPLMSetDataf(gPlanePsi, (float)yaw);
	...
	XPLMSetDataf(gPlaneX, (float)(x + 1));
