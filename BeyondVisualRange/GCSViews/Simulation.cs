...
namespace ArdupilotMega.GCSViews
{
    public partial class Simulation : MyUserControl
    {
        ...
        int simPort = 49000;    // Data from Ground Station to X-Plane
        int recvPort = 49005;   // Data from X-Plane to Ground Station
        ...
        private void mainloop()
        {
            ...
            while (threadrun == 1)
            {
               // if ((comPort.BaseStream.IsOpen == false) && (MainV2.comPort.logreadmode==false)) { continue; } //slv 4/16/13
                ...
                if (SimulatorRECV.Available > 0)
                {
                    ...
                    try
                    {
                        while (SimulatorRECV.Available > 0)
                        {
                            ...
                            //Console.WriteLine("Getting data from X-Plane");
                            hzcount++;
                        }
                    }
                    catch
                ...
                if ((comPort.BaseStream.IsOpen == false) && (false)) { break; }
                try
                {
                    ...
                    if ((DateTime.Now - simsendtime).TotalMilliseconds > 19)
                    {
                        ...
                        //Console.WriteLine("Getting data from GCS");
                        processArduPilot();
                    }
                }
        ...
        private void SetupUDPRecv()
        {
            ...
            OutputLog.AppendText("Listening on port UDP " + recvPort + " (sim->planner)\n");
        }
        ...
        private void RECVprocess(byte[] data, int receviedbytes, ArdupilotMega.MAVLink comPort)
        {
            ...
            //Console.Write("RECVprocess() X-Plane->GS\n  ");
            if (data[0] == 'D' && data[1] == 'A')
            {
                ...
                float centripaccel;
                double head;
                YLScsDrawing.Drawing3d.Vector3d accel3D;
                YLScsDrawing.Drawing3d.Vector3d cent3D;

                turnrad = (float)(((DATA[3][7] * 0.44704) * (DATA[3][7] * 0.44704) * 1.943844) / (float)(11.26 * Math.Tan(att.roll))) * ft2m;
                centripaccel = (float)((DATA[3][7] * 0.44704) * (DATA[3][7] * 0.44704)) / turnrad;
                ...
                if (xplane9)
                {
                    ...
                    accel3D = HIL.QuadCopter.RPY_to_XYZ(DATA[18][1], DATA[18][0], 0, -9.8);
                    cent3D = HIL.QuadCopter.RPY_to_XYZ(DATA[18][1] - 90, 0, 0, centripaccel);
                    head = DATA[18][2] - 90;
                }
                else
                {
                    ...
                    accel3D = HIL.QuadCopter.RPY_to_XYZ(DATA[17][1], DATA[17][0], 0, -9.8);
                    cent3D = HIL.QuadCopter.RPY_to_XYZ(DATA[17][1] - 90, 0, 0, centripaccel);
                    head = DATA[17][2] - 90;
                }
...
//                Console.WriteLine("{0:0.00000} {1:0.00000} {2:0.00000} \t {3:0.00000} {4:0.00000} {5:0.00000}", pdiff, rdiff, ydiff, DATA[17][0], DATA[17][1], DATA[17][2]);
                ...
                imu.xacc = (Int16)(DATA[4][5]*1000);
                imu.yacc = (Int16)(DATA[4][6]*1000);
                imu.zacc = (Int16)(DATA[4][4]*1000);
                ...
                if (xplane9)
                {
                    gps.cog = ((ushort)DATA[19][2]); //((float)DATA[19][2]);
                }
                else
                {
                    gps.cog = ((ushort)DATA[18][2]); //((float)DATA[18][2]);
                }
...
//                sitlout.thetadot = att.rollspeed;
            ...
            if (chkSensor.Checked == false) // attitude
            {
                ...
                comPort.sendPacket(asp);
                comPort.sendPacket(imu);    // SLV_ADDED 04-24-2013
            }
            else // raw imu
            {
                // imudata
...
#if SLV_ADDED
        /// <param name="flaps_out">0 to 1</param>
        private void updateScreenDisplay(double lat, double lng, double alt, double roll, double pitch, double heading, double yaw, double roll_out, double pitch_out, double rudder_out, double throttle_out, double flaps_out)
        {
            try
            {
                // Update Sim stuff
        ...
        private void processArduPilot()
        {
            ...
            //Console.Write("processArduPilot() GS->X-Plane\n");
            if (CHK_quad.Checked && !RAD_aerosimrc.Checked)
            {
                ...
                m[2] = (ushort)MainV2.cs.ach3out;
                m[3] = (ushort)MainV2.cs.ch4out;
                m[4] = (ushort)MainV2.cs.ch6out;
#else
...
#if SLV_ADDED
                updateScreenDisplay(lastfdmdata.latitude, lastfdmdata.longitude, lastfdmdata.altitude * .3048, lastfdmdata.phi, lastfdmdata.theta, lastfdmdata.psi, lastfdmdata.psi, m[0], m[1], m[2], m[3], m[4]);
#else
...
#if SLV_ADDED
            float roll_out, pitch_out, throttle_out, rudder_out, collective_out, flaps_out;
#else
            ...
            if (heli)
            {
                roll_out = (float)MainV2.cs.aileron_servo / (float)rollgain;
                pitch_out = (float)MainV2.cs.elevator_servo / (float)pitchgain;
                throttle_out = (float)Math.Abs((float)MainV2.cs.throttle_servo / (float)throttlegain);
                rudder_out = (float)MainV2.cs.rudder_servo / (float)ruddergain;
#if SLV_ADDED
                flaps_out = (float)MainV2.cs.hilch6 / (float)ruddergain;
#endif
                collective_out = (float)(MainV2.cs.throttle_servo - 1500) / throttlegain;
            }
            else
            {
                roll_out = MainV2.cs.aileron_servo / 100.0f;
                pitch_out = MainV2.cs.elevator_servo / 100.0f;
                throttle_out = Math.Abs(MainV2.cs.throttle_servo) / 100.0f;
                rudder_out = MainV2.cs.rudder_servo / 100.0f;
#if SLV_ADDED
                flaps_out = MainV2.cs.hilch6 / 100.0f;
#endif
...
#if SLV_ADDED
            flaps_out = Constrain(flaps_out, 0, 1);
#endif
            ...
            try
            {
                ...
                {
#if SLV_ADDED
                    if (RAD_softXplanes.Checked) // Data coming from x-plane to AP
                    {
                        ...
                        if (xplane9)
                        {
                            if ((DATA[18] != null) && (DATA[19] != null) && (DATA[20] != null))
                                updateScreenDisplay(DATA[20][0] * deg2rad, DATA[20][1] * deg2rad, DATA[20][2] * .3048, DATA[18][1] * deg2rad, DATA[18][0] * deg2rad, DATA[19][2] * deg2rad, DATA[18][2] * deg2rad, roll_out, pitch_out, rudder_out, throttle_out, flaps_out);
                        }
                        else
                        {
                            if ((DATA[18] != null) && (DATA[19] != null) && (DATA[20] != null))
                                updateScreenDisplay(DATA[20][0] * deg2rad, DATA[20][1] * deg2rad, DATA[20][2] * .3048, DATA[17][1] * deg2rad, DATA[17][0] * deg2rad, DATA[18][2] * deg2rad, DATA[17][2] * deg2rad, roll_out, pitch_out, rudder_out, throttle_out, flaps_out);
                        }
                    }

                    if (RAD_softFlightGear.Checked || RAD_JSBSim.Checked)
                    {
                        updateScreenDisplay(lastfdmdata.latitude, lastfdmdata.longitude, lastfdmdata.altitude * .3048, lastfdmdata.phi, lastfdmdata.theta, lastfdmdata.psi, lastfdmdata.psi, roll_out, pitch_out, rudder_out, throttle_out, flaps_out);
                    }

                    if (RAD_aerosimrc.Checked)
                    {
                        updateScreenDisplay(aeroin.Model_fLatitude * deg2rad, aeroin.Model_fLongitude * deg2rad, aeroin.Model_fPosZ, aeroin.Model_fRoll, aeroin.Model_fPitch, aeroin.Model_fHeading, aeroin.Model_fHeading, roll_out, pitch_out, rudder_out, throttle_out, flaps_out);
                    }
#else
                    if (RAD_softXplanes.Checked)
                    {
            ...
            if (RAD_aerosimrc.Checked)
            {
                ...
                if (CHK_quad.Checked)
                {
                    ...
                    Array.Copy(BitConverter.GetBytes((double)((MainV2.cs.ach3out - 1100) / 800 * 2 - 1)), 0, AeroSimRC, 0, 8); // motor 1 = front
...
#if SLV_ADDED
                byte[] Xplane = new byte[5 + 36 + 36 + 36];
                if (heli)
                {
                    Xplane = new byte[5 + 36 + 36 + 36 + 36];
                }
#else
...
#if SLV_ADDED
                Array.Copy(BitConverter.GetBytes((float)(rudder_out * REV_rudder * 0.5)), 0, Xplane, 61, 4);
#else
...
#if SLV_ADDED
                // NEXT ONE - control surfaces
                Array.Copy(BitConverter.GetBytes((int)13), 0, Xplane, 77, 4); // packet index
                Array.Copy(BitConverter.GetBytes((int)0), 0, Xplane, 81, 4); // start data
                Array.Copy(BitConverter.GetBytes((int)0), 0, Xplane, 85, 4);
                Array.Copy(BitConverter.GetBytes((int)0), 0, Xplane, 89, 4);
                Array.Copy(BitConverter.GetBytes((float)flaps_out), 0, Xplane, 93, 4);
                Array.Copy(BitConverter.GetBytes((float)flaps_out), 0, Xplane, 97, 4);
                Array.Copy(BitConverter.GetBytes((int)0), 0, Xplane, 101, 4);
                Array.Copy(BitConverter.GetBytes((int)0), 0, Xplane, 105, 4);
                Array.Copy(BitConverter.GetBytes((int)0), 0, Xplane, 109, 4);
