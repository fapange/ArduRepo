...
namespace ArdupilotMega.GCSViews
{
     ...
     * partial class UDPpacket
    {
        ...
        public double Elevation;    //Feet
        public double Vx;
        public double Vy;
        public double Vz;
        public byte chkSumHi;
        public byte chkSumLo;
        public Byte[] na = new Byte[2];
        public UDPpacket() { }
        ~UDPpacket() { }
    }
     * **/
    ...
    partial class FlightData : MyUserControl
    {
        ...
        int tickStart = 0;
#if SLV_ADDED
        //UDPpacket recvPacket = new UDPpacket();
        const int NPOINTS = 600;
        RollingPointPairList list1  = new RollingPointPairList(NPOINTS);
        RollingPointPairList list2  = new RollingPointPairList(NPOINTS);
        RollingPointPairList list3  = new RollingPointPairList(NPOINTS);
        RollingPointPairList list4  = new RollingPointPairList(NPOINTS);
        RollingPointPairList list5  = new RollingPointPairList(NPOINTS);
        RollingPointPairList list6  = new RollingPointPairList(NPOINTS);
        RollingPointPairList list7  = new RollingPointPairList(NPOINTS);
        RollingPointPairList list8  = new RollingPointPairList(NPOINTS);
        RollingPointPairList list9  = new RollingPointPairList(NPOINTS);
        RollingPointPairList list10 = new RollingPointPairList(NPOINTS);
#else
        ...
        public FlightData()
        {
            ...
            cpolygons = new GMapOverlay(gMapControl1, "cpolygons");
            gMapControl1.Overlays.Add(cpolygons);

            croutes = new GMapOverlay(gMapControl1, "croutes");
            ...
            catch (Exception ex) { Console.WriteLine("GspeedMAX exception " + ex.ToString()); }*/
        }
        ...
        private void mainloop()
        {
            ...
            while (threadrun == 1)
            {
                ...
                if (!(lastdata.AddSeconds(8) > DateTime.Now) && comPort.BaseStream.IsOpen)
                {
                    ...
                    catch (Exception ex) { Console.WriteLine("REQ streams exception " + ex.ToString()); }
                ...
                catch (Exception ex) { Console.WriteLine("aviwriter exception " + ex.ToString()); }*/

                /*
                try
                {
                    if (MainV2.cs.RC_state == 128)
                    {
                ...
                catch (Exception ex) { Console.WriteLine("FailSafe Button exception " + ex.ToString()); }
                */
                ...
                /*if (MainV2.comPort.logreadmode && MainV2.comPort.logplaybackfile != null)
                {
                    ...
                    catch (Exception ex) { Console.WriteLine("comport readPacket exception " + ex.ToString()); }
                    ...
                    catch (Exception ex) { Console.WriteLine("avi play exception " + ex.ToString()); }
                    if (ts > 0)
                ...
                try
                {
                    ...
                    if (MainV2.trafficSocket.Available>0)
                    {
                        ...
                        while (npackets > 0)
                        {
                            ID[0] = buffer[4];  ID[1] = buffer[5];  ID[2] = buffer[6];  ID[3] = buffer[7];
                            ID[4] = buffer[8];  ID[5] = buffer[9];  ID[6] = buffer[10]; ID[7] = buffer[11];
                            Latitude = BitConverter.ToDouble(buffer, 12);    //Degrees
                            Longitude = BitConverter.ToDouble(buffer, 20);    //Degrees
                            Elevation = BitConverter.ToDouble(buffer, 28);    //Feet
                            Vx = BitConverter.ToDouble(buffer, 36);
                            Vy = BitConverter.ToDouble(buffer, 44);
                            Vz = BitConverter.ToDouble(buffer, 52);
                        #if true
                            newTraffic = true;
                            for (trafficIdx = 0; ((trafficIdx < MainV2.trafficTable.Count) && newTraffic); trafficIdx++)
                            {
                                if (MainV2.trafficTable[trafficIdx].ID.Equals(BitConverter.ToString(ID)))
                                {
                                    newTraffic = false;
                                }
                            }
                            if (newTraffic)
                            {
                                aname = BitConverter.ToString(ID);
                                MainV2.trafficTable.Add(new MainV2.Aircraft(aname, (float)Latitude, (float)Longitude, (float)Vz));
                                Console.Write("                                              Added  : "); Console.WriteLine(aname);
                            }
                            else
                            {
                                MainV2.trafficTable[trafficIdx - 1].Update((float)Latitude, (float)Longitude, (float)Vz);
...
#if SLV_nADDED
                    if (tracklast.AddSeconds(0.1) < DateTime.Now)
#else
...
#endif
                    {
                        ...
                        //{
                            ...
                            if (waypoints.AddSeconds(10) < DateTime.Now)
                            {
                                ...
                                // Candidate Flight Plan from StratWay
                                // v------Starts Here
/*
                                if (MainV2.comPort.cwps != null)
                                {
                                    // this is for the pulled wp list from stratway
                                    FlightPlanner.cpointlist.Clear();
                                    FlightPlanner.cpointlist.AddRange(MainV2.comPort.cwps);
                                }
                                cpolygons.Markers.Clear();

                                foreach (PointLatLngAlt plla in FlightPlanner.cpointlist)
                                {
                                    if (plla == null || plla.Lng == 0 || plla.Lat == 0)
                                        break;
                                    //addpolygonmarker(plla.Tag, plla.Lng, plla.Lat, (int)plla.Alt, plla.color, cpolygons);
                                    addpolygonmarker(plla.Tag, plla.Lng, plla.Lat, (int)plla.Alt, Color.Crimson, cpolygons);
                                }
                                RegenerateCandidatePolygon();
*/
                                // ^------Ends Here
                                // Candidate Flight Plan from StratWay
...
#if SLV_ADDED
#if true
                                    //Console.Write("Traffic Table Length: "); Console.WriteLine(MainV2.trafficTable.Count);
                                    for (int i = 0; i < MainV2.trafficTable.Count; i++)
                                    {
                                        //Console.Write(MainV2.trafficTable[i].ID); Console.WriteLine(":");
...
#endif
                                    routes.Markers.Add(new GMapMarkerPlane(currentloc, MainV2.cs.yaw, MainV2.cs.groundcourse, MainV2.cs.nav_bearing, MainV2.cs.target_bearing, gMapControl1) { ToolTipText = MainV2.cs.alt.ToString("0"), ToolTipMode = MarkerTooltipMode.Always });
#else
                                    routes.Markers.Add(new GMapMarkerPlane(currentloc, MainV2.cs.yaw, MainV2.cs.groundcourse, MainV2.cs.nav_bearing, MainV2.cs.target_bearing));
#endif
                                }
        ...
        private void updatePlayPauseButton(bool playing)
        {
            ...
            else
            {
                this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate()
                {
                    ...
                    catch (Exception ex) { Console.WriteLine("playlog exception " + ex.ToString()); }
                });
            }*/
        }
        ...
        private void updateBindingSource()
        {
            this.BeginInvoke((System.Windows.Forms.MethodInvoker)delegate()
            {
                ...
                catch (Exception ex) { Console.WriteLine("updateBinding exception " + ex.ToString()); }
            });
        }
        ...
        private void updateMapPosition(PointLatLng currentloc)
        {
            this.BeginInvoke((MethodInvoker)delegate()
            {
                ...
                catch (Exception ex) { Console.WriteLine("updateMapPosition exception " + ex.ToString()); }
            });
        }
        ...
        private void updateMapZoom(int zoom)
        {
            this.BeginInvoke((MethodInvoker)delegate()
            {
                ...
                catch (Exception ex) { Console.WriteLine("updateMapZoom exception " + ex.ToString()); }
            });
        }
        ...
        private void updateLogPlayPosition()
        {
            /*this.BeginInvoke((MethodInvoker)delegate()
            {
                ...
                catch (Exception ex) { Console.WriteLine("updateLogPlayPosition exception " + ex.ToString()); }
            });*/
        }
        ...
        private void addpolygonmarker(string tag, double lng, double lat, int alt, Color? color, GMapOverlay overlay)
        {
            try
            {
                ...
                if (color != Color.LightBlue)
                {
                    ...
                    catch (Exception ex) { Console.WriteLine("mBorder exception " + ex.ToString()); }
            ...
            catch (Exception ex) { Console.WriteLine("addPolygonMarker exception " + ex.ToString()); }
        }
        ...
        void RegeneratePolygon()
        {
            ...
            else
            {
                ...
                polygon.Stroke = new Pen(Color.Yellow, 3);
        ...
        void RegenerateCandidatePolygon()
        {
            ...
            else
            {
                ...
                {
                    gMapControl1.UpdatePolygonLocalPosition(cpolygon);
                }
            }
        }
        ...
        private void BUTzero_acc_hold_Click(object sender, EventArgs e)
        {
            try
            {
                ((Button)sender).Enabled = false;

                comPort.doAction(MAVLink.MAV_ACTION.MAV_ACTION_RESET_ACC_MAX_MIN); // set nav from
                System.Threading.Thread.Sleep(100);
            }
            catch { MessageBox.Show("The command failed to execute"); }
            ((Button)sender).Enabled = true;
        ...
        private void BUTFailSafe_Emergency_Click(object sender, EventArgs e)
        {
            try
            {
                ((Button)sender).Enabled = false;
        ...
        private void Zoomlevel_ValueChanged(object sender, EventArgs e)
        {
            ...
            catch (Exception ex) { Console.WriteLine("zoom level change exception " + ex.ToString()); }
        }
        ...
        private void gMapControl1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                catch (Exception ex) { Console.WriteLine("map move mouse exception " + ex.ToString()); }
            }
        }
        ...
        {
            MainV2.cs.altoffsethome = (float)MainV2.cs.HomeLocation.Alt;
        }*/
         ...
         /*private void tabPage1_Resize(object sender, EventArgs e)
        {
            ...
            if (tabGauges.Width < 500)
            {
                ...
                Gthrottle.Height = mywidth;
                Gfwdcurrent.Height = mywidth;
                Grud.Height = mywidth;

                Grpm.Height = mywidth;
                GaccX.Height = mywidth;
                GaccY.Height = mywidth;
                GaccZ.Height = mywidth;

                Gspeed.Location = new Point(0, 0);
                Galt.Location = new Point(Gspeed.Right + 1, Gspeed.Top);
                Gheading.Location = new Point(Galt.Right+1, Galt.Top);

                Gthrottle.Location = new Point(Gspeed.Left, Gspeed.Bottom + 1);
                Gfwdcurrent.Location = new Point(Gthrottle.Right + 1, Gthrottle.Top);
                Grud.Location = new Point(Gfwdcurrent.Right + 1, Gfwdcurrent.Top);

                Grpm.Location = new Point(Gthrottle.Left, Gthrottle.Bottom + 1);
                GaccX.Location = new Point(Grpm.Left, Gthrottle.Bottom + 1);
                GaccY.Location = new Point(GaccX.Right + 1, Gthrottle.Bottom + 1);
                GaccZ.Location = new Point(GaccY.Right + 1, Gthrottle.Bottom + 1);
            }
            else
            {
                Gvspeed.Visible = true;
                Gaftcurrent.Visible = true;
                ...
                Gspeed.Location = new Point(Gvspeed.Right + 1, Gvspeed.Top);
                Galt.Location = new Point(Gspeed.Right + 1, Gspeed.Top);
                Gheading.Location = new Point(Galt.Right + 1, Galt.Top);


                Gthrottle.Location = new Point(Gvspeed.Left, Gvspeed.Bottom + 1);
                Gfwdcurrent.Location = new Point(Gthrottle.Right + 1, Gthrottle.Top);
                Gaftcurrent.Location = new Point(Gfwdcurrent.Right + 1, Gfwdcurrent.Top);
                Grud.Location = new Point(Gaftcurrent.Right + 1, Gaftcurrent.Top);

                Grpm.Location = new Point(Gthrottle.Left, Gthrottle.Bottom + 1);
                GaccX.Location = new Point(Grpm.Right + 1, Gthrottle.Bottom + 1);
                GaccY.Location = new Point(GaccX.Right + 1, Gthrottle.Bottom + 1);
                GaccZ.Location = new Point(GaccY.Right + 1, Gthrottle.Bottom + 1);
            }
        ...
        void zg1dropout_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        private void tabControl1_SelectedIndexChanged(object sender, EventArgs e)
        {
            /*if (Tabs.SelectedTab == tabStatus)
            {
                ...
                foreach (var field in test.GetProperties())
                {
                    ...
                    lbl1.Size = new System.Drawing.Size(125, 20);
                    lbl1.Text = field.Name;
                    lbl1.Name = field.Name;
                    lbl2.AutoSize = false;

                    lbl2.Location = new Point(lbl1.Right + 2, y);
        ...
        private void zg1_DoubleClick(object sender, EventArgs e)
        {
            if (!zg1dropout)
            {
                splitContainer1.Panel1Collapsed = true;
                Form dropout = new Form();
                dropout.Size = new System.Drawing.Size(zg1.Width, zg1.Height + 20);
                splitContainer1.Panel1.Controls.Remove(zg1);
                dropout.Controls.Add(zg1);
                //dropout.Resize += new EventHandler(dropout_Resize);
                dropout.FormClosed += new FormClosedEventHandler(zg1dropout_FormClosed);
                dropout.Show();
                zg1dropout = true;
            }
        }
...
";
        ...
        private void CDnR_Send_Click(object sender, EventArgs e)
        {
            /*MAVLink.__mavlink_cdnr_controller_t req;
            req.h_flag = (byte)CDnR_CheckHeading.CheckState;
            req.s_flag = (byte)CDnR_CheckAirspeed.CheckState;
            req.a_flag = (byte)CDnR_CheckAltitude.CheckState;
            req.t_flag = (byte)CDnR_CheckTime.CheckState;
            req.new_heading  = (Int16)CDnR_Heading.Value;
            req.new_airspeed = (Int16)CDnR_Airspeed.Value;
            req.new_altitude = (Int16)CDnR_Altitude.Value;
            req.max_time     = (Int16)CDnR_Time.Value;
            MainV2.comPort.sendCDnR(req);*/
        }

        private void simSendMessage_Click(object sender, EventArgs e)
        {
            /*MAVLink.__mavlink_traffic_sim_timing_t req;
            req.sim_time = (float)trafficSimTime.Value;
            req.time_to_wpt = (float)trafficTimeToWpt.Value;
            MainV2.comPort.sendTrafficSimTiming(req);*/
        }
    }
}