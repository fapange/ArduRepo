    ...
    partial class Firmware : MyUserControl
    {
...
#if SLV_ADDED
            this.pictureBoxLaRCCode.Image = ArdupilotMega.Properties.Resources.planeicon;
#endif
        ...
        internal void Firmware_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (XmlTextReader xmlreader = new XmlTextReader("https://raw.github.com/diydrones/binary/master/Firmware/firmware2.xml"))
                {
...
#if SLV_ADDED
                pictureBoxLaRCCode.Text = "Custom AP Code";
#endif
...
#if SLV_ADDED
        private void pictureBoxLaRCCode_Click(object sender, EventArgs e)
        {
            software temp = new software();
            OpenFileDialog openFileDialog1 = new OpenFileDialog();

            //UploadFlash(Path.GetDirectoryName(Application.ExecutablePath) + Path.DirectorySeparatorChar + @"firmware.hex", board);
            openFileDialog1.InitialDirectory = Path.GetDirectoryName(Path.GetDirectoryName(Application.StartupPath)) + Path.DirectorySeparatorChar + "Firmware";
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    temp.desc = openFileDialog1.SafeFileName.ToString();
                    temp.name = openFileDialog1.SafeFileName.ToString();
                    temp.url = openFileDialog1.FileName.ToString();
                    temp.url2560 = openFileDialog1.FileName.ToString();
                    temp.url2560_2 = openFileDialog1.FileName.ToString();
                    temp.k_format_version = 12;

                    //pictureBoxLaRCCode.Text = temp.name;
        ...
        private void localupdate(software temp)
        {
            string board = "";
            string baseurl = "";
